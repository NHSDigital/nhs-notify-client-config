...
const $ClientPublishedEventMetadata = $EventMetadata.extend({
  ...
  dataschema: z.string().regex(/^https:\/\/notify\.nhs\.uk\/events\/client-config\/client-published-1.\d+\.\d+\.json$/),
  dataschemaversion: z.string().regex(/^1\.\d+\.\d+$/), // Matches semantic versioning format with fixed major version
});

export const $ClientPublishedEvent = $ClientPublishedEventMetadata.extend({
  data: $Client,
